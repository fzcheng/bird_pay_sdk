...
public class DBOpenHelper extends SQLiteOpenHelper {
	...
	private void createPushTable(SQLiteDatabase db){
		db.execSQL("CREATE TABLE IF NOT EXISTS "+PushDatabase.TABLE_NAME+"("
				+ PushDatabase.COL_ID + " integer primary key autoincrement, "
				+ PushDatabase.COL_TITLE + " varchar(100), "
				+ PushDatabase.COL_MESSAGE + " varchar(1024), "
				+ PushDatabase.COL_TYPE + " integer, "
				+ PushDatabase.COL_IS_READ + " integer, "
				+ PushDatabase.COL_MSG_ID + " varchar(100), "
				+ PushDatabase.COL_MSG_TIME + " varchar(100), "
				+ PushDatabase.COL_URL + " varchar(256), "
				+ PushDatabase.COL_RESERVER + " varchar(1024))");
	}
