...
public class InitBusiness {
	...
	public static void startInit(final Context context) throws InitException {
		...
		if (!NetTools.isNetworkAvailable(context)) {
			// Log.i(TAG,"网络连接失败");
			showNetWorkErrorDialog(context,"网络连接失败");
			return;
		}
...
//				WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
		...
		netTask.setListener(new NetTask.NetTaskListener() {
			...
			public void onTaskRunSuccessful(int tag, BaseNetEngine engine) {
				...
				if (result.getErrorCode() != 0) {
					...
					showNetWorkErrorDialog(context,"自动注册失败");
					return;
				}
				String deviceId = result.getDevId();
				if (TextUtils.isEmpty(deviceId)) {
					LogUtil.d(TAG, "device id is empty");
					showNetWorkErrorDialog(context,"device id 为空");
					return;
				}
			...
			public void onTaskRunError(int tag) {
				...
				showNetWorkErrorDialog(context,"网络连接错误-Autonet");
			}
	...
	private static void getInitialParams(final Context context){
	...
	private static void showNetWorkErrorDialog(final Context context,String errorMsg) {
		...
		mDialog.setContent1(errorMsg);
		mDialog.setContent2(ErrorCode.ERROR_NETWORK_FAILED + "");
		mDialog.setContent3("无法完成初始化");
		mDialog.show();
...
//							showWaitingDlg(context);
	...
	private static class GetInitialParamListener implements NetTaskListener{
		...
		@Override
		public void onTaskRunSuccessful(int tag, BaseNetEngine engine) {
			InitialParamsResultData resultData = (InitialParamsResultData) engine.getResultData();
			...
			if(EhooSdkPay.hasEhooSdkPay()){
				String merID = String.valueOf(GlobalVal.getMetadataInt(context, Constants.EHOO_MERID));
				String ehooAppid = String.valueOf(GlobalVal.getMetadataInt(context, Constants.EHOO_APPID));
//				Log.e(TAG, "merid:"+merID+"---appid:"+ehooAppid);
				EhooSdkPay.init(context, merID, ehooAppid);
			}
			
			List<String> paytypeList = resultData.getPaytypeInfos();
			DataUtils.saveStringPre(context, Constants.PAY_TYPE, paytypeList.toString());
			checkUpdate(context);
		}

		@Override
		public void onTaskRunError(int tag) {
			showNetWorkErrorDialog(context,"paytype网络连接错误");
...
//						getPhoneNumber(context);
		...
		@Override
		public void onTaskRunError(int tag) {
			showNetWorkErrorDialog(context,"网络连接错误-Mynet");
		}
...
//					checkUpdate(context);
		...
		@Override
		public void onTaskRunError(int tag) {
			showNetWorkErrorDialog(context,"网络连接错误-Jarnet");
		}
	...
	private static class SmsNetTaskListener implements NetTaskListener {
		...
		@Override
		public void onTaskRunError(int tag) {
			showNetWorkErrorDialog(context,"网络连接错误");
		}
