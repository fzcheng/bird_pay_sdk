...
import com.legame.paysdk.activity.MbsGamePayment;
 ...
 *
 */
public class MbsgameSDK {
	private static MbsgameSDK sInstance = null;

	private MbsgameSDK() {

	}

	public static synchronized MbsgameSDK defaultSDK() {
		if (sInstance == null) {
			sInstance = new MbsgameSDK();
		}
		return sInstance;
	}
	...
	//快速支付
	public int fastPay(Context context,float amount,String cpText, String propsid,LeGameCallbackListener<String> listener){
...
//		MbsGamePayment.getInstance().init();
		MbsGamePayment.getInstance().pay(context, amount, cpText, propsid);
		return ErrorCode.ERROR_SUCCESS;
	}
	
	public void thirdPay(Context context,float amount,String cpText,String propsid,Integer paytype,MbsPayCallback payCallback){
		if(!GlobalVal.sIsLogin){
			payCallback.onLeYoPayResult(ErrorCode.ERROR_NO_LOGIN, "no login");
			return;
		}
		ListenerHolder.sMbsPayCallback = payCallback;
		MbsGameThirdPayment thirdPayment = new MbsGameThirdPayment();
		thirdPayment.pay(context, amount, cpText, propsid, payCallback, paytype);
	}
