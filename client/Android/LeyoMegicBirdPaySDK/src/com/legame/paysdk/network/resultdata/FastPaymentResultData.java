...
public class FastPaymentResultData extends BaseResultData {
	...
	private Map<String, String> mapEhoo;
...
//					Log.i(TAG,"fast--name-->"+name);
					...
					} else if (name.equals("order")) {//{type=, pay_info=未找到可用快捷支付方式, order_no=}
						map = getXmlAttributes(parser);
						mType = map.get("type");
						order = new Order();
...
//						Log.i(TAG,"代计费的游戏名："+map.get("gamename"));
					...
					}else if(name.equals("ehoo")){
						mapEhoo = getXmlAttributes(parser);
					}else{
	...
	public Map<String,String> getMapEhoo(){
		return mapEhoo;
	}
