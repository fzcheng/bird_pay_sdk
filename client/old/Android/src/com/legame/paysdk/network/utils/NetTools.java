...
public class NetTools {
	...
	public static String getServerUrl(){
		if(GlobalVal.sDebug) 
		{
			//return "http://192.168.16.233:8080/api";
//			return "http://dev.leyogame.cn/api";
			return "http://leyo.magicbirds.cn/api";
//			return "http://192.168.1.99:8888/api";//测试链接
		} else {
...
//		{//测试环境
//			if(!gIsDomainFailure){
////				return "http://192.168.1.99/api";
////				return "http://192.168.10.137/api";//测试链接
//				return "http://dev.leyogame.cn/api";
//			}else{
//				if(!gIsNetConnect){
//					gTryIndex++;
//				}
//				if(gTryIndex <= gListURLinfo.size()){
////					System.out.println("gTryIndex2--》"+gTryIndex);
//					return gListURLinfo.get(gTryIndex-1).getDomain();
//				}else{
//					System.out.println("没有一个域名有效");
//					gIsNetFailure = true;
//					return null;
//				}
//			}
//		} else {//正式环境
//			if(!gIsDomainFailure){//没有链接失效的情况下
//				if(!gIsFirstStart){//不是第一次链接
//					if(!gIsNetConnect){
//						gTryIndex++;
//					}
//					if(gTryIndex <= gListURLinfo.size()){
//						return gListURLinfo.get(gTryIndex-1).getDomain();
//					}else{
//						LogUtil.d("Domain Name", "没有一个域名有效:测试环境");
//						gIsNetFailure = true;
//						return null;
//					}
//				}else{//第一次启动
//					LogUtil.d("Domain Name", "第一次启动软件，默认使用so文件封装的链接");
//					return NetLib.getServerUrl(); //正式服务器，后期需要使用lib形式进行封装	
//				}
//			}else{
//				gIsDomainFailure = false;
//				if(!gIsNetConnect){
//					gTryIndex++;
//				}
//				if(gTryIndex <= gListURLinfo.size()){
//					return gListURLinfo.get(gTryIndex-1).getDomain();
//				}else{
//					LogUtil.d("Domain Name", "没有一个域名有效");
//					gIsNetFailure = true;
//					return null;
//				}
//			}	
