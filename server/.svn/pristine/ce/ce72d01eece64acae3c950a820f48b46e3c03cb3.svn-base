...
public class Bind_phone_sms extends AbstractSdkClientService<CmdVerifyPhoneSms> {
	...
	protected Result execute(CmdVerifyPhoneSms cmd) {
		...
		try{			 
			...
			pv.setUid(gameSession.getUid());
			pv.setCreateTime(new Date());
			pv.setType(1);
			pv.setVcode(vcode);				
			
			 
			String msgContent = Cfg.msg.getString("sdk.client.bind_phone.sms");
