...
//			}
			...
			while (its.hasNext()) {
				...
				for (int k = 0, methodObjSize = methodObj.size(); k < methodObjSize; k++) {
					...
					if(index%2==0)
						cell.setCellStyle(getTwoStyle(workbook,style));
					else
						cell.setCellStyle(getOneStyle(workbook,style));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static HSSFCellStyle getTwoStyle(HSSFWorkbook workbook,HSSFCellStyle style){
		// 产生Excel表头
		// 产生Excel表头
		 //HSSFCellStyle style = workbook.createCellStyle(); 
	...
	public static HSSFCellStyle getOneStyle(HSSFWorkbook workbook,HSSFCellStyle style){
		// 产生Excel表头
		// 产生Excel表头
		 //HSSFCellStyle style = workbook.createCellStyle(); 
		 style.setBorderLeft((short)1);     //左边框
