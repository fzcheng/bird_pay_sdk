...
public class VoidExample{
	...
	public static void main(String[] args){     
		...
		req.put("transType", "31");// 交易类型
		req.put("merId", UpmpConfig.MER_ID);// 商户代码
		req.put("backEndUrl", UpmpConfig.MER_BACK_END_URL);// 通知URL
		req.put("orderTime", new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()));// 交易开始日期时间yyyyMMddHHmmss（撤销交易新交易日期，非原交易日期）
		req.put("orderNumber", generateOrdrNo());// 订单号（撤销交易新订单号，非原订单号）
