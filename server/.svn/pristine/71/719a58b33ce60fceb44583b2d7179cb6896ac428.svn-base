...
$(function(){
    ...
    onBeforeEdit:function(index,row){
	    ...
	    $('#${config_id}List').datagrid('refreshRow', index);
	...
	function ${config_id}Listsearch(){
		...
		$('#${config_id}List').<#if config_istree=="Y">treegrid<#else>datagrid</#if>(
			{
				url:'exCgAutoListController.do?datagrid&configId=${config_id}&field=${fields}'
			}
		);
	}
	function dosearch(params){
		...
		$('#${config_id}List').<#if config_istree=="Y">treegrid<#else>datagrid</#if>(
			{
				url:'exCgAutoListController.do?datagrid&configId=${config_id}&field=${fields},',queryParams:jsonparams
			}
		);
	}
	function ${config_id}Listsearchbox(value,name){
		...
		alert("queryParams"+queryParams+" " + value);
	...
	function clickSearch(){
		...
		var field_id = $(".${config_id}ListSearch").val();
		
        field_id = "."+field_id;
        
		$(field_id).find('input').each(
			function(){
			
				if($(this).attr('name') != null){
					queryParams.searchfield = $(this).attr('name');
					queryParams[$(this).attr('name')]=$(this).val();
				}
			}
		);
	...
	function deleterow(index){
	    ...
	    $.messager.confirm('Confirm','Are you sure?',function(r){
	        ...
	        if (r){
	            ...
	            $('#${config_id}List').datagrid('deleteRow', index);
	...
	function saverow(index){
	    ...
	    $('#${config_id}List').datagrid('endEdit', index);
	
	}

	function cancelrow(index){
	
	    $('#${config_id}List').datagrid('cancelEdit', index);
...
<#if  (config_queryList?size >0)>
	...
	<#if config_querymode == "single">
		<span style="float:right" class="${config_id}ListSingle">
		
		<select class="${config_id}ListSearch" style="float: left;
				...
				<#list config_queryList  as x>
					<option value="${x['field_id']}">${x['field_title']}</option>
				</#list>
		</select>
		
				
		<#list config_queryList  as box>
			<#if box['field_showType'] == 'list' || box['field_showType'] == 'checkbox' || box['field_showType'] == 'radio'>
				<div class="${config_id}ListsingleSearch ${box['field_id']}" style="float:left">
					<select  class="easyui-combobox sbox" name = "${box['field_id']}">
							<#list box['field_dictlist']  as dict>
								<option value="${dict['TYPECODE']}">${dict['TYPENAME']}</option>
							</#list>
					</select>
			   </div>
			<#elseif box['field_showType'] == 'date' || box['field_showType'] == 'datetime'>
				<div class="${config_id}ListsingleSearch ${box['field_id']}" style="float:left">			
			       			<input class="easyui-datebox sbox " name = "${box['field_id']}_begin"></input>
			       			<span>~</span>
							<input class="easyui-datebox sbox" name = "${box['field_id']}_end" ></input>
				</div>
			<#else>
			       <#if box['field_type'] == 'Integer' || box['field_type'] == 'Long'>
			       		<div class="${config_id}ListsingleSearch ${box['field_id']}" style="float:left">
			       			<input  class="easyui-validatebox sbox" name = "${box['field_id']}_begin" data-options="iconCls:'icon-ok'" type="text" style="width: 63px;height: 20px;overflow: hidden;padding: 0;vertical-align: middle;white-space: nowrap;   background-color: #FFFFFF;border-color: #95B8E7;line-height: 20px;border-style: solid;border-width: 1px;display: inline-block;"/>
							<span>~</span>
							<input  class="easyui-validatebox sbox" name = "${box['field_id']}_end" data-options="iconCls:'icon-ok'" type="text" style="width: 63px;height: 20px;overflow: hidden;padding: 0;vertical-align: middle;white-space: nowrap;   background-color: #FFFFFF;border-color: #95B8E7;line-height: 20px;border-style: solid;border-width: 1px;display: inline-block;"/>
			       		</div>
			       <#else>
			       		<div class="${config_id}ListsingleSearch ${box['field_id']}" style="float:left">
			       			<input  class=" easyui-validatebox sbox" name = "${box['field_id']}"  data-options="iconCls:'icon-ok'" type="text" style="width: 63px;height: 20px;overflow: hidden;padding: 0;vertical-align: middle;white-space: nowrap;   background-color: #FFFFFF;border-color: #95B8E7;line-height: 20px;border-style: solid;border-width: 1px;display: inline-block;"/>
			       		</div>
			       </#if>
			
			</#if>
		</#list>
		<input type="button" class="easyui-input icon-search l-btn-icon-left" onclick="clickSearch()" style="cursor:pointer;float:right;width: 20px;height: 22px;overflow: hidden;padding: 0;vertical-align: middle;white-space: nowrap;   background-color: #FFFFFF;border-color: #95B8E7;line-height: 20px;border-style: solid;border-width: 1px;display: inline-block;"/>
					
		<!--
		<input id="${config_id}Listsearchbox" class="easyui-searchbox"  data-options="searcher:${config_id}Listsearchbox,width:'10px',prompt:'请输入关键字',menu:'#${config_id}Listmm'" "></input>
...
<script type="text/javascript">
	$(function(){
			$(".${config_id}ListSearch option").eq(0).attr('selected', 'true');
    		var field_id1 = $(".${config_id}ListSearch").val();
        	$(".${config_id}ListsingleSearch").hide();
        	field_id1 = "."+field_id1;
        	$(field_id1).show();
        	$(".${config_id}ListSearch").change(function(){
