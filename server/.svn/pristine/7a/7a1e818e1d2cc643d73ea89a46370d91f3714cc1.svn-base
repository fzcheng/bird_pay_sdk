...
public class JeecgSqlUtil {
	...
	public static String getMethodSql(String methodUrl) {
		...
		if(ConstantsDefs.MODE_DEVELOP.equals(ResourceUtil.getConfigByName("sqlReadMode"))){
			//update-start--Author:zhangdaihao  Date:20130810 for：SQL文件读取支持Jar
			return getMethodSqlLogicJar(methodUrl);
			//update-end--Author:zhangdaihao  Date:20130810 for：SQL文件读取支持Jar
		}
		//[2].发布模式：pub SQL文件只加载一次
		else if(ConstantsDefs.MODE_PUBLISH.equals(ResourceUtil.getConfigByName("sqlReadMode"))){
...
//		String projectPath =  request.getSession().getServletContext().getRealPath("/");
	...
	public static void main(String[] args) {
		//System.out.println(getAppPath(JeecgSqlUtil.class));
		System.out.println(getCountSqlBySql("SELECT * 	from JEECG_DICT_PARAM WHERE 1=1"));
	}
