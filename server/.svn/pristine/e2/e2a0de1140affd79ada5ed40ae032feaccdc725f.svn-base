...
INSERT INTO sdk_operation_remain(game_id,stat_day,cnt_new) 
(
...
INSERT INTO sdk_operation_remain(game_id,stat_day,cnt_remain_2) 
(
...
INSERT INTO sdk_operation_remain(game_id,stat_day,cnt_remain_3) 
(
...
INSERT INTO sdk_operation_remain(game_id,stat_day,cnt_remain_7) 
(
...
/*15day*/
INSERT INTO sdk_operation_remain(game_id,stat_day,cnt_remain_15) 
(
SELECT  G.game_id, @nowTime AS stat_day ,COUNT(U.uid) AS cnt_remain_15  FROM  sdk_user U  JOIN sdk_user_login_game SL ON SL.uid=U.uid  
JOIN sdk_game G ON G.game_id=SL.game_id
WHERE  DATE_FORMAT(SL.last_login_time,	'%m-%d-%Y') = DATE_FORMAT(@nowTime,	'%m-%d-%Y')
AND  DATE_FORMAT(SL.first_login_time,	'%m-%d-%Y') = DATE_FORMAT(DATE_ADD(@nowTime, INTERVAL -15 DAY),	'%m-%d-%Y')
GROUP BY G.game_id
)
ON DUPLICATE KEY UPDATE cnt_remain_15=VALUES(cnt_remain_15);

/*30day*/
INSERT INTO sdk_operation_remain(game_id,stat_day,cnt_remain_30) 
(
