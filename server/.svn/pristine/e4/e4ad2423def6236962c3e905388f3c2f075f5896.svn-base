...
import org.jeecgframework.core.interceptors.CusLocalSessionFactoryBean;
...
public abstract class GenericBaseCommonDao<T, PK extends Serializable>
	...
	private volatile boolean isChangeSessionFactory = false;
  ...
  public Session getSession(){
    ...
    if (this.isChangeSessionFactory) {
      if (this.updateSession == null) {
        try {
          this.updateSession = this.sessionFactory.openSession();
        } catch (Exception e) {
          this.updateSession = this.sessionFactory.getCurrentSession();
        }
  ...
  // 设置session
  public void setSession(DataSource dataSource){
//    try
//    {
////      i = 1 / 0;
//    }
...
//      e.printStackTrace();
//    }
    CusLocalSessionFactoryBean sessionFactoryBean = new CusLocalSessionFactoryBean();
    ...
    this.isChangeSessionFactory = true;
  }
  ...
  private <T> void getProperty(Class entityName){
	...
	public <T> T findUniqueByProperty(Class<T> entityClass,
		...
		return (T) createCriteria(entityClass, new Criterion[] { 
	...
	public <T> List<T> findByProperty(Class<T> entityClass,
			String propertyName, Object value){
    Assert.hasText(propertyName);
    return (List<T>) createCriteria(entityClass, new Criterion[] { 
...
//	    return (T) t;
	...
	public <T> List<T> findByPropertyisOrder(Class<T> entityClass,
	    ...
	    Assert.hasText(propertyName);
	    return createCriteria(entityClass, isAsc, new Criterion[] { 
	      Restrictions.eq(propertyName, value) }).list();
	}
	...
	public Integer getRowCount(DetachedCriteria criteria){
	    return Integer.valueOf(oConvertUtils.getInt(((Criteria)criteria
	...
	public PageList getPageList(final CriteriaQuery cq,  final boolean isOffset){
	    ...
	    int curPageNO = PagerUtil.getcurPageNo(allCounts, cq.getCurPage().intValue(), 
	      pageSize);// 当前页
    ...
    return new DataTableReturn(Integer.valueOf(allCounts), Integer.valueOf(allCounts), cq.getDataTables()
  ...
  public DataGridReturn getDataGridReturn(final CriteriaQuery cq,
	...
	public PageList getPageListBySql(final HqlQuery hqlQuery,
	    ...
	    List list = null;
	    if (isToEntity){
	      list = (List)ToEntityUtil.toEntityList(query.list(), 
...
//	    Object po = null;
	...
	public Long getCountForJdbc(String sql){
		return Long.valueOf(this.jdbcTemplate.queryForLong(sql));
	}
	...
	public Long getCountForJdbcParam(String sql, Object[] objs){
		return Long.valueOf(this.jdbcTemplate.queryForLong(sql, objs));
	}
  ...
  public Integer countByJdbc(String sql, Object[] param) {
    return Integer.valueOf(this.jdbcTemplate.queryForInt(sql, param));
  }
  ...
  public Map<String, Object> findOneForJdbc(String sql, Object[] objs) {
	 ...
	 * @return
	 */
	public <T> List<T> findPageListByHql(String hql, int page, int rows, Object[] param){
	    Query q = getSession().createQuery(hql);
	    if ((param != null) && (param.length > 0)) {
	      for (int i = 0; i < param.length; i++) {
	        q.setParameter(i, param[i]);
