...
$(function(){$('#${config_id}List').datagrid(
	...
	idField: 'id',
	title: '${config_name}',
	url:'channelStat.do?datagrid&configId=${config_id}&query_param=${default_query_param}',
	fit:true,
	fitColumns:true,
	pageSize: 10,
	pagination:true,
	...
	columns:[
		...
		<#list config_fieldList  as x>  
			{field:'${x['field_name']}',title:'${x['field_txt']}',width:50,sortable:'true' ,<#if x['field_type'] == "hidden" >hidden:true,</#if>},
		</#list>
		]
	],
	...
	function ${config_id}Listsearch(){var queryParams=$('#${config_id}List').datagrid('options').queryParams;$('#${config_id}Listtb').find('*').each(function(){queryParams[$(this).attr('name')]=$(this).val();});$('#${config_id}List').datagrid({url:'channelStat.do?datagrid&configId=${config_id}&query_param=${default_query_param}'});}
	function dosearch(params){var jsonparams=$.parseJSON(params);$('#${config_id}List').datagrid({url:'channelStat.do?datagrid&configId=${config_id},',queryParams:jsonparams});}
	...
	//导出
	function exportXls() {
		var submitUrl = "channelStat.do?exportXls&configId=${config_id}&query_param=${default_query_param}";
		var queryParams = "";
