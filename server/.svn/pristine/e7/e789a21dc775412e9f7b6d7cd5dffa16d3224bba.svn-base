		...
		[	
			<#if config_istree=="Y">
				<#list config_fieldList  as x>  
					...
					<#if x_index!=0>{field:"${x['field_id']}", title:"${x['field_title']}",<#if x['field_isShow'] == "N" || x['field_showType']=="file">hidden:true,</#if><#if x['field_href'] != "">formatter:function(value,rec,index){var href='';href+="<a href='#' onclick=\"addOneTab('字段链接','${x['field_href']}')\" ><u>"+value+"</u></a>";return href;},</#if> width:100}, </#if>
				</#list>
			<#else>
					...
					<#list config_fieldList  as x>  
						 ...
						 <#if x['field_isShow'] == "N" || x['field_showType']=="file">hidden:true,</#if>
						 <#if x['field_href'] != "">
						 	formatter:function(value,rec,index){
						 		var href='';
						 		href+=applyHref('${x['field_href_title']}','${x['field_href']}',value,rec,index);
						 		return href;
						 	},
...
<#if config_querymode == "group">
	...
	<a href="#" class="easyui-linkbutton" plain="true" icon="icon-add" onclick="addTable('${config_name}录入','exCgFormBuildController.do?ftlForm&tableName=${config_id}','${config_id}List',${style_width},${style_height})">${config_name}录入</a>
