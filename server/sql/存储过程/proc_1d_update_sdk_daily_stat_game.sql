...
DROP PROCEDURE IF EXISTS `proc_1d_update_sdk_daily_stat_game`$$

CREATE DEFINER=`root`@`%` PROCEDURE `proc_1d_update_sdk_daily_stat_game`()
BEGIN
	-- -----------------------------------------------------
	-- 设置前一天时间
	-- 定时器每日凌晨3:30点执行一次
	-- 重新统计上一天的支付信息，防止计费回调延迟比较长的情况
	-- 执行该脚本时，先执行cron_day_daily_stat_game_channel.sql
