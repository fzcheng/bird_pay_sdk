...
public class ListtoMenu {
	...
	private static String getLeafOfTree(TSFunction function){
		...
		String furl=function.getFunctionUrl();
		if(furl.startsWith("http://") || furl.startsWith("https://")){
			menuString.append(furl);
			if(furl.indexOf("isIframe")<0){
				if(furl.indexOf("?")>0){
		...
		menuString.append("\',\'");
		menuString.append(icon);
		menuString.append("\'); return false;\"  title=\"");
		menuString.append(function.getFunctionName());
		menuString.append("\" url=\"");
		menuString.append(function.getFunctionUrl());
		menuString.append("\" href=\""+function.getFunctionUrl()+"\"><span class=\"nav\" >");
		menuString.append(function.getFunctionName());
		menuString.append("</span></a></li>");
		return menuString.toString();
	}
	
	//update-start--Author:周勇  Date:2013-07-31：新增bootstrap头部菜单导航,多级菜单
	/**
	 * 拼装bootstrap头部菜单
	 * @param pFunctions
	 * @param functions
