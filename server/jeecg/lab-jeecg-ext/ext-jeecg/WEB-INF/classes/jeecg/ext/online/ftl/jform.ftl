					...
					<td class="value">
					  ...
					  <#elseif po.show_type=='readonly'>
						...
						<#elseif po.show_type=='richtext'>
							<textarea id="${po.field_name}" name="${po.field_name}" 
							       style="width: ${po.field_length}px" class="inputxt" onchange="myWindValueChange('${po.field_name}')" onclick="myWindow('${po.field_name}', 'richtext')"
					               <#if po.is_null == 'Y'>ignore="ignore"</#if>
					               <#if po.field_valid_type?if_exists?html != ''>datatype="${po.field_valid_type?if_exists?html}"</#if>>${data['${tableName}']['${po.field_name}']?if_exists?html}</textarea>
...
/plugin/passwordStrength/passwordStrength-min.js">
	...
	<div id="myRichTextWind" style="display:none;width:100%;height:100%"><textarea id="myRichTextWindValue"></textarea></div>
	<script type = "text/javascript">
	$(function(){
			var myRichTextEditor = document.getElementById("myRichTextWindValue");
			CKEDITOR.replace(myRichTextEditor);
	});
	</script>
	  ...
	  function myWindow(field, show_type){
	  		if(show_type != 'richtext'){
					...
					$("#myRichTextWind").window({
					    ...
					    onClose: function(){
					    		var text = $("#myRichTextWindValue").val(); 
			  					$("#"+ field).val(text);
			  					$("#myRichTextWindValue").val(""); 
			  				},
					    });
					    $("#myRichTextWind").show();
					    $("#myRichTextWindValue").show();
