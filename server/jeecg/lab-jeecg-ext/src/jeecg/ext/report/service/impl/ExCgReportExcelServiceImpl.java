...
@Service(value="exCgReportExcelService")
@Transactional
public class ExCgReportExcelServiceImpl extends CommonServiceImpl implements
		ExCgReportExcelServiceI {
...
//			Iterator itTitle = titleSet.iterator();
			...
			//遍历内容
			int maxRowOneSheet =60000;
			int count = 0;
			int sheetNum = 1;
			while (itData.hasNext()) {

				
				if(count == maxRowOneSheet){
					 for(int i=0; i<titleList.size(); i++){
		                 sheet.autoSizeColumn(i); 
					 }
					sheet = workbook.createSheet(title+String.valueOf(sheetNum++));
					rindex = 0;
					cindex = 0;
					// 产生表格标题行
					Row row0 = sheet.createRow(rindex);
					row.setHeight((short) 450);
					titleStyle = getTitleStyle(workbook);
					for(Map titleM : titleList){
						String titleContent = (String) titleM.get("field_txt");
						Cell cell = row0.createCell(cindex);
						RichTextString text = new HSSFRichTextString(titleContent);
						cell.setCellValue(text);
						cell.setCellStyle(titleStyle);
						cindex++;
					}
					bodyStyle = getOneStyle(workbook);
					count = 1;
