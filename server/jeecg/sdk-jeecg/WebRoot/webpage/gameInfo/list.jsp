...
function setFufeiWeizhi(id){
 ...
 <div region="center" style="padding:1px;">
  <t:datagrid name="gameDataList" title="游戏信息列表"  actionUrl="gameInfo.do?datagrid&cp_id=${cp_id}" fit="true" idField="gameId" queryMode="group">
   ...
   <t:dgCol title="创建时间" field="createTime"  formatter="yyyy-MM-dd hh:mm:ss" width="15"></t:dgCol>
    ...
    <t:dgConfOpt title="删除" url="gameInfo.do?del&gameId={id}&cp_id=${cp_id}" message="此操作将会删除与之相关的所有数据，确认删除？" ></t:dgConfOpt>  
   	<t:dgFunOpt title="查看付费信息"  funname="openFei(id)" ></t:dgFunOpt> 
   	<t:dgFunOpt title="微派计费设置"  funname="setWeipai(id)" ></t:dgFunOpt>
   	<t:dgFunOpt title="付费位置"  funname="setFufeiWeizhi(gameId)" ></t:dgFunOpt>
   	<t:dgToolBar title="添加" icon="icon-add" funname="add" url="gameInfo.do?addorupdate&cp_id=${cp_id}" ></t:dgToolBar>
   	<t:dgToolBar title="编辑" icon="icon-edit" funname="update" url="gameInfo.do?addorupdate&cp_id=${cp_id}" ></t:dgToolBar>
    <t:dgToolBar title="查看" icon="icon-search" funname="detail" url="gameInfo.do?addorupdate&cp_id=${cp_id}"></t:dgToolBar>
  </t:datagrid>
 </div>
</div>