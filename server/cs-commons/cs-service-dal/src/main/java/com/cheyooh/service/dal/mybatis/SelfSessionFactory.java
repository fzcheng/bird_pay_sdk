...
public class SelfSessionFactory implements SqlSessionFactory {
	private static final Log logger = LogFactory.getLog(SelfSessionFactory.class);
	...
	public SelfSessionFactory(XmlConfiguration configuration) {		 
	...
	private SqlSession openSessionFromDataSource(ExecutorType execType, TransactionIsolationLevel level, boolean autoCommit) {
		try {
			...
			return new DBSqlSession(configuration, executor, autoCommit);
	...
	private DataSource getDataSourceFromEnvironment(Environment environment) {
		if (environment == null || environment.getDataSource() == null) {
			throw new SqlSessionException(
					"Configuration does not include an environment with a DataSource, so session cannot be created unless a connection is passed in.");
		}
